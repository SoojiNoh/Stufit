...
Event.create(
    ...
    content:'서울특별시 교육부에서는 청소년들의 꿈을 이야기������� 수 있는 꿈을 잇(IT)다 미디어 페스티벌을 개최합니다. 여러분들의 많은 관심 부탁드립니다.',
...
MajorStory.create(
    ...
    q1:
        '(독일어학과 vs 독일언어문화학과)
    >>> 독일어학과의 경우에는 독일어(어학, 회화, 번역 등)에 초점을 두���서 공부하는 것으로 알고 있어!! 반면에 독일언어문화학과는 과 이름에서 볼
    ...
    보통 두 분야를 선택하는데 나는 문화와 문학 수업을 많이 선택해서 들었어. 3, 4 학��� 때도 자신이 선택한 분야에 대한 과목들을 선택해서 수강하
...
MajorStory.create(
    ...
    전산이론, 정보이론 등이 있어요.',
    q2:
        '1학년: 고등학교 때 배웠던 수학(미적분, 벡터...)을 꼼꼼히 정리하는 ���간이에요.
    ...
    q3:
        '학교가 아기자기하면서 예쁘고, 동기들도 다들 열심히 해서 서로 도와가며 공부할 수 있어 만족합니다! 또한 복수전공을 자유롭게 할 수 있어서 관심분야를 같이 공부해 나갈 수 있���는 장점도 있어���!
        >>> 만족도(5점 만점): 3.7점',
...
MajorStory.create(
    ...
    전공필수 과목은 위에 언급된 게 전부라서 2학년부터는 전공선택 과목 중에서 듣고 싶은 과목을 각자 골라가며 배워.
        2학년: 2학년군에 해당되는 전공선택 과목들의 특징은 이론을 ���우되 실습 같은 과제(!)로 좀 더 깊이있게 배운다는 거야. 공모전과 연계하는 등의 과제를 수행하는 경우도 많거든.
...
MajorStory.create(
    ...
    그래서, 컴퓨터의 성격, 사고하는 방식에 대한 감을 잡으면 학과공부를 하는데 수월할 것 같아요. 어떻게 보면 컴퓨터가 사고하는 방식과 인간이 사고���는 방식이 비슷해요. 라면 끓이는 레시피를 한
...
토지이용에 관한 내용을 배워. 또, 추가적으로 컴퓨터로 지리정보를 ���게
...
MajorStory.create(
    ...
    q1:
        '모두 이미 알고 있겠지만 간호학을 배우는 과야. 간호국가���시를 볼 수있는 자격��� 갖추기 위해

필요��� 과목과 학점을 이수하고 실습도 나가지. 간호학과의 졸업시험이 곧 간호면허시험이라고 보면
...
MajorStory.create(
    ...
    아닌, 어떤 이슈에 대해 토론하고 생각하고 글로 풀���내고 말로 설득하는 능력��� 중요한 공부를 하기 때문이지.',
    #university_id:
    created_at: '2016-08-23')
#20번
MajorStory.create(
    ...
    우리 과 선배님이셔.',
    q2:
        '1학년 때는 주로 이론 수업��� 들어. 커뮤니케이션 이론이라는 수업이 가장 대표적인데, 여기서 배우는 내용이 다른 전공 수업에서도 꽤 많이 언급돼.
...
MajorStory.create(
    ...
    소설학회, 연극학회 등 다양하게 갖춰져 있어. 또한 학부생은 교직이수를 신청하거나 스토리텔링 연계전공과 ���제한국어교육연계전공도 선택할 ��� 있어.',
    q2:
        '한국어���학부의 수업은 학년이 낮다고 아예 고학년의 수업을 듣지 못하는 구조는 아니지만
...
MajorStory.create(
    ...
    이외에도 중국의 문학에 대한 다양한 수업을 들을 수 있어. 고전문학부터 현대문학, 시와 소설 등 다양한 분야의 문학수업이 마련되어있기 때문에 관심있는 부분을 골라서 수강할 수 있어.
        -3학년: 2학년때보다 더 심도있고 폭넓은 공부를 하게 돼. 중국의 문화까지 ���부하는 시기야
        -4학년: 고급회화 수업과 중국어 통번역 수업을 들으��� 실무적인 역량을 키우는 수업을 듣게 돼. 또한 실제 실무경험에 대한 특강 수업도 마련되어있어서 실무상황에 대해 간접적으로 체험할 수 있어.',
    q3:
    ...
    학회와 원어연극 활동에 참여함으로써 중국에 대한 흥미를 이끌어낼 ��� 있어.
        >>> 만족도(5점 만점): 3.5점',
...
MajorStory.create(
    ...
    remote_file_url: 'http://i.imgur.com/W8T81fa.png',
    q1:
        '우리학교는 크게 4가지 분야로 나누어져 있어. 디자인/소재/의복구성/마케팅으로 구성되어���는데 디자인 분야에서는 한스타일 분야와 서양복 분야를 나누어 배운다는 것이 다른 학교와 다른
    ...
    q2:
        ...
        2학년 때부터 많은 것이 달라져. 일학년 때 필수 교양을 거의 채워놓고 2학년부터는 전공에 집중하게 되는데 이때부터 바쁜 전공생활이 시작돼. 그���지만 모든 분야의 기초를 배우는 시기이기 때문에 나름
...
있어서 나는 그런 부분은 별로 걱정하지 않지만 일단 휴학을 하지 않고 졸업���다는 가정 하에 다른

학교 학생들보다 공부를 1년 덜하고 사회로 나간다는건 조금 단점��� 될 수도 있다고 생각해(그래서 좀
...
학교���도 있고 건축학과의 소속이 예술대, 미대, 디자인대학에 속해있는 경우도 많아. 사실 건축학은

엄밀히 구분하자면 공과대학에도 속할 수 없고 예술대학에도 속할 수 없어. 개념이 좀 광범위하거든.

그런데 공대에 있다 보니 미적분학이���던가 과학이라던가 그런 내 전공과는 연관이 거��� 없는 것들도

몇몇수업은 필수적으로 수강해���하더라고.
...
MajorStory.create(
    ...
    나가는 사람이 있어도 ���가로 정규직을 뽑지 않거나 계약직을 고용하는 경우가 많아. 사서e마을의 고용게시판을 보면 정규직 채용공고에 비해 계약직 채용공고가 몇 배로 많은 걸로도 알 수 있는
...
침구학, 임상과목인 한방재활의학, 사���의학 등의 과목을 학습한다.
...
본과 4학년: 대학 부속 한방병원과 임상술기센터에서 실습을 진행하고 한의사면허시험을 준비한다.',
    q3:
        '지금���지 배운 지식을 한의사가 되어 환자들에게 베풀 때까지 ���족은 잠시 미뤄두기로....^^!!
        >>> 만족도(5점 만점): 4.5점',
...
과목들을 배움으로써 이론에서부터 산업에서 쓰이는 공정까지 다양한 학���을 배우게 됩니다.
...
있어.',
    q2:
        '1학년: 전공과목보다 글쓰기나 외국어 등 교양과목을 더 많이 듣게 되��� 거 같애.
...
고��� 때는 전교 3등안에 무조건 들어야 해. 최대한 열심히 해서 내신 올리고 교내상이 중요하니깐 교내

수학경시대회 같은 교내���회에 잘 참가해야 해. 그리고 이 전형은 자기주도로 해서 성적이 상승곡선을
